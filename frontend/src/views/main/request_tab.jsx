                          ...
                          <button
                            ...
                            onClick={async () => {
                              ...
                              const fetchApprovalRequests = async () => {
                                const petRes = await fetch.getPet();
                                if (petRes.data) {
                                  const userPets = petRes.data.filter(item => item.pet && item.pet.user_id === user.user.user_id).map(item => item.pet);
                                  const adoptionReqRes = await fetch.getAdoptionRequest();
                                  if (adoptionReqRes.data) {
                                    const petMap = {};
                                    userPets.forEach(pet => { petMap[pet.pet_id] = pet; });
                                    const requestsNeedingApproval = adoptionReqRes.data.filter(req => petMap[req.pet_id]).map(req => ({ ...req, pet: petMap[req.pet_id] }));
                                    setApprovalRequests(requestsNeedingApproval);
                                  }
                                }
                              };
                  ...
                  adoptionRequests.map((req) => (
                    <div key={req.req_id} className="flex flex-col md:flex-row mb-6 border-b pb-6 last:border-b-0 last:pb-0">
                      ...
                      <div className="mb-4 mt-6">
                        ...
                        <p className="text-base text-gray-700 font-semibold">Requested by: <span className="font-normal">{user?.user.user_name || '-'}</span></p>
                      </div>
                    </div>
                  ))
                )}
